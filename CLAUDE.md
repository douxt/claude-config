...
3. 在 worktree 内开发测试提交：
     ...
     b. 改代码 → 浏览器热加载验证 → 改到满意
     c. **禁止 `npm run build` 等构建命令**，dev server 热加载已足够验证
     d. `wt commit <任务名> "消息"` 提交
     e. `wt dev-stop` 停止开发环境
     f. **停止并告知用户**：改动已提交，请在 worktree 中手动测试验证。**不主动合并**，等用户明确指示。
4. **用户明确要求合并时**，合入 master：① `git status` 确认改动 → ② 按项目跑编译/语法检查 → ③ `git diff master...HEAD` 审查改动量，**确认无意外回退或多余文件** → ④ 功能验证 → ⑤ `git pull --rebase origin master` → ⑥ 分支 `git rebase origin/master`（**冲突时立即停止**，列出冲突文件，等用户手动解决后再继续）→ ⑦ `git checkout master && git merge <分支>` → ⑧ 合后 `git diff --stat master@{1} master` 确认无代码回退 → ⑨ `git push origin master` → ⑩ 清理 worktree 与分支

> ①②③④ 在 rebase 前执行。冲突后重复 ②③④。
>
> **合并安全铁律**：
> - 合前分支必须 rebase 到最新 master；**rebase 冲突不能跳过、不能自动解决**，必须人工介入
> - 合后必须 `git diff master@{1} master` 确认无意外代码回退
> - 合并后立即推送，不在本地滞留 merge 结果
> - stash pop 冲突不可跳过、必须先解决再继续
> - **合并完成后显式报告用户**：合入了哪些文件、改动量（+N/-N 行）、有无冲突及处理结果
